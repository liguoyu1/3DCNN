...
########## train
  ...
  # ########## CONFIG_FILE
  CONFIG_FILE=${CODE_ROOT}/MultiModal-Tagging/configs/config.tagging.5kresnet_fusion_attention.yaml
  if [ -z "$2" ]; then
...
########## test
  ...
  OUTPUT_FILE_PATH="./results/tagging_5k_fusion_attention.json"
  if [ -z "$3" ] ;then
